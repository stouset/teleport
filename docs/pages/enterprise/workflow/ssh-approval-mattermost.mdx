...
The above sequence should result in three PEM-encoded files being generated: `auth.crt`, `auth.key`, and `auth.cas` (certificate, private key, and CA certs respectively). We'll reference these files later when [configuring the plugin](#configuring-mattermost-bot).
</TabItem>
<TabItem title="Cloud" scopes={["cloud"]}>
```code
$ tctl auth sign --user=access-plugin-mattermost --out=auth.pem --ttl=2190h
# ...
```

The above sequence should result in one identity file, `auth.pem`. We'll reference this file later when [configuring the plugin](#configuring-mattermost-bot).
</TabItm>
</Tabs>
...
<Tabs>
<TabItem scopes={["oss", "enterprise"]} title="Self-Hosted">
```yaml
(!examples/resources/plugins/teleport-mattermost-self.toml!)
```
</TabItem>
<TabItem scopes={["cloud"]} title="Cloud">
```yaml
