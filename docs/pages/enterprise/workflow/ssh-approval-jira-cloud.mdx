...
Log into the Teleport Authentication Server where you normally run `tctl`. Create a
...
The Jira Cloud Teleport plugin uses the `access-plugin-jira` role and user to perform the approval. We will export the identity filesusing [`tctl auth sign`](../../setup/reference/cli.mdx#tctl-auth-sign).

<Tabs>
<TabItem title="Self-Hosted" scopes={["oss", "enterprise"]}>

```bash
tctl auth sign --format=tls --user=access-plugin-jira --out=auth --ttl=2190h
# ...
```
The above sequence should result in three PEM encoded files being generated: `auth.crt`, `auth.key`, and `auth.cas` (certificate, private key, and CA certs respectively). We'll reference these files later when [configuring the plugins](#configuration-file).

</TabItem>
<TabItem title="Cloud" scopes={["cloud"]}>

```bash
tctl auth sign --user=access-plugin-jira --out=auth.pem --ttl=2190h
# ...
```

This command will generate an identity file called `auth.pem`. We'll reference this file later when [configuring the plugins](#configuration-file).
</TabItem>
</Tabs>
